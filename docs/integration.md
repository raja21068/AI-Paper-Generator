...
| `researchclaw init` | Interactive config creation (choose LLM provider, creates `config.yaml`) |
...
Add the `metaclaw_bridge` section to your `config.yaml`:
...
researchclaw run --config config.yaml --topic "Your idea" --auto-approve
...
bash scripts/start-metaclaw.sh
```
...
| `scripts/start-metaclaw.sh` | Helper script to launch MetaClaw proxy |
